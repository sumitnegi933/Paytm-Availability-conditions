...
//        }
    ...
    public function save() {
        $result = (object)array('type' => 'paytm');
...
//        }
     ...
     * by JavaScript code.
     *
     * @param string $businessemail The email of paytm to be credited
    ...
    public static function get_json($currency, $cost) {
        return (object)array('type' => 'paytm', 'currency' => $currency, 'cost' => $cost);
    }
    ...
    public function is_available($not, \core_availability\info $info, $grabthelot, $userid) {
        global $DB;
        // Should double-check with paytm everytime ?
        $context = $info->get_context();
        $allow = $DB->record_exists('availability_paytm_tnx',
                                  array('userid' => $userid,
    ...
    protected function get_either_description($not, $standalone, $info) {
        $context = $info->get_context();
        $url = new \moodle_url('/availability/condition/paytm/view.php?contextid='.$context->id);
        if ($not) {
            return get_string('notdescription', 'availability_paytm', $url->out());
        } else {
                $a = new \stdClass();
                $a->cost = $this->currency.' '.$this->cost;
                $a->url =$url->out();
            return get_string('eitherdescription', 'availability_paytm', $a);
        }
    }
